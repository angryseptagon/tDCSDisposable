...
bool chk_volt(){
  ...
  if (latest_voltage < 53){
    ...
    return false;
    Serial.print("voltage check failed");
}
}
...
analogWrite16(9,level);
chk_volt();
if (!stimfail && (latest_voltage < 50)){
  Serial.println("begining ramp");
  rampstart(30);//ramp arguement is duration of ramp in seconds
  Serial.println("ending ramp,begining stimulation");
  stimulation(30);//stimulation arduement in seconds
